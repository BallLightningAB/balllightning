Ball Lightning AB is the maintained company presence, portfolio, and project archive for software, integrations, and digital projects by Nicolas Brulay. Nicolas works full-time at Hultafors Group as Product & Service Owner - Integrations, with a public focus on integration ownership, modernization, harmonization, strategy, AI, automation, and long-term technical capability development.
...
Keep Ball Lightning AB available as a company reference point, selected-work portfolio, project archive, and home for selected non-competing side projects. The site is maintained, but it is not currently used to accept new client engagements.

## Tech Stack

TanStack Start, TailwindCSS v4, shadcn/ui, Biome, and Ultracite.
...
- **CI secret scanning**: The GitHub Actions workflow `.github/workflows/secret-scan.yml` runs Gitleaks on pushes and pull requests to `main`, uploading results to the repository security tab.
...
This project uses [zod](https://zod.dev/) for runtime validation and type inference. A central helper is defined in `src/lib/validation/zod.ts`. When adding schemas for env, loaders, or forms, import `z` from there:
